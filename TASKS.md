...
- `witness_byte_record_eq_matches_fields_case_boundary_shift` — `property_byte_record_eq_matches_fields(vec![b"12", b"34"], vec![b"123", b"4"])` → `Pass`. Under `byte_record_eq_field_boundaries_efc4a51_1` the two records compare equal because the fallback compares concatenated field bytes.
- `witness_byte_record_eq_matches_fields_case_length_mismatch` — `property_byte_record_eq_matches_fields(vec![b"12", b"34", b"56"], vec![b"12", b"34"])` → `Pass`. Under `byte_record_eq_length_check_23fb0cd_1` the shorter record compares equal because the zip-based field check silently truncates at the shorter iterator.
